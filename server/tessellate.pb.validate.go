...
// Validate checks the field values on UnlockDeadRequest with the rules defined
// in the proto definition for this message. If any rules are violated, an
// error is returned.
func (m *UnlockDeadRequest) Validate() error {
	...
	if utf8.RuneCountInString(m.GetToken()) < 1 {
		return UnlockDeadRequestValidationError{
			Field:  "Token",
			Reason: "value length must be at least 1 runes",
		}
	}
...
// UnlockDeadRequestValidationError is the validation error returned by
// UnlockDeadRequest.Validate if the designated constraints aren't met.
...
// Error satisfies the builtin error interface
func (e UnlockDeadRequestValidationError) Error() string {
	cause := ""
	if e.Cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.Cause)
	}
	...
	return fmt.Sprintf(
		"invalid %sUnlockDeadRequest.%s: %s%s",
		key,
		e.Field,
		e.Reason,
		cause)
...
var _ error = UnlockDeadRequestValidationError{}
